...
module.exports = (req, res) => {
    new Promise((resolve, reject) =>{
        ...
        resolve => {
            ...
            let query = ' category_id = 3 AND quantity > 0 AND ';
            let count = 0;
            ...
            let getProducts = new Promise((resolve, reject) => {
                let SQLquery = 'SELECT id, name, short_description, description, price, purchase_price, product_url, img_url, quantity, vendor, category_id, attr_manufacturer, attr_capacity, provider_num FROM products WHERE ' + query;
            ...
            Promise.all([getProducts, getAttr, returnSelectedAttr, returnAttrJSON, getSEOText]).then(
                ...
                seo.title = "Магазин МКПП | Атомобильные омыватели";
                seo.text = "";
                ...
                if (resolve[0].length > 20){
                    let numPages = Math.ceil(resolve[0].length / 20);
                        pagesPaginator.push({
                            'name' : '<< В начало',
                            'href' : '/catalog/washerliquid/'+ req.params.sub_url + '?page=1',
                            isActive : false
                        })
                    if(req.query.page) {
                        let count = 0;
                        ...
                        while( count < 7 && count < numPages && startPage < numPages + 1){
                            if(startPage > 0){
                                if(startPage == req.query.page){
                                    pagesPaginator.push({
                                        'name' : startPage,
                                        'href' : '/catalog/washerliquid/'+ req.params.sub_url + '?page=' + startPage,
                                        isActive : true
                                    })
                                } else{
                                    pagesPaginator.push({
                        ...
                        if(numPages > 6){
                            for(let i = 2; i < 8; i++){
                                pagesPaginator.push({
                                    'name' : i,
                                    'href' : '/catalog/washerliquid/'+ req.params.sub_url + '?page=' + i,
                                    isActive : false
                                })
                            }
                        }else{
                    ...
                    pagesPaginator.push({
                        'name' : 'В конец >>',
                        'href' : '/catalog/washerliquid/'+ req.params.sub_url + '?page=' + numPages,
                        isActive : false
                    })
                }
                let products;
                ...
                reject => {
                    log.info('some errors in handling Promise.all() catalog-washerliquid-filter' + reject);
                }
            )
        },
        reject => {
