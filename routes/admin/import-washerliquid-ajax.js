...
module.exports = function(req, res, next){
    importWasherliquid(res);
}

function importWasherliquid(res){
    var promiseGetProductsFromExelFile = new Promise ((resolve, reject) => {
    var workbook = new Excel.Workbook();
    workbook.xlsx.readFile('./import/washer_liquid.xlsx')
        .then(
            (data) => {
                var importProducts = [];
                ...
                for(var i = 1; i < rows.length; i++){
                    ...
                    if (rows[i]['_cells'][0]['_value']['value'] != null && rows[i]['_cells'][1] != undefined){
                        currProd.short_description = rows[i]['_cells'][1]['_value']['value'];
                    } else{
                        currProd.short_description = '';
                    }
                    if (rows[i]['_cells'][0]['_value']['value'] != null && rows[i]['_cells'][2] != undefined){
                        currProd.description = rows[i]['_cells'][2]['_value']['value'];
                    } else{
                    ...
                    if (rows[i]['_cells'][0]['_value']['value'] != null && rows[i]['_cells'][4] != undefined){
                        currProd.img_url = '/static/catalog/' + currProd.category_id + '/'+ rows[i]['_cells'][4]['_value']['value'];
                    } else{
                        currProd.img_url = '';
                    }
                    if (rows[i]['_cells'][0]['_value']['value'] != null && rows[i]['_cells'][5] != undefined){
                        currProd.quantity = rows[i]['_cells'][5]['_value']['value'];
                    } else{
                        currProd.quantity = '';
                    }
                    if (rows[i]['_cells'][0]['_value']['value'] != null && rows[i]['_cells'][6] != undefined){
                        currProd.vendor = rows[i]['_cells'][6]['_value']['value'].toString().replace(/\s/g, '').toLowerCase();
                    } else{
                        currProd.vendor = '';
                    }
                    if (rows[i]['_cells'][0]['_value']['value'] != null && rows[i]['_cells'][8] != undefined){
                        currProd.attr_manufacturer = rows[i]['_cells'][8]['_value']['value'];
                    } else {
                    ...
                    if (rows[i]['_cells'][0]['_value']['value'] != null && rows[i]['_cells'][9] != undefined){
                        currProd.attr_capacity = rows[i]['_cells'][9]['_value']['value'];
                    } else {
            ...
            Promise.all(promiseArr).then(
                resolve => {
                    ...
                    for(var i = 0; i < products.length; i++){
                        if(existedVendors.indexOf(products[i]['vendor']) == -1){
                            ...
                            promiseArr.push(
                                new Promise( (result, error) => {
                                    var connection = manage.createConnection();
                                    var SQLquery = "INSERT INTO import (name, short_description, description, price, category_id, img_url, quantity, vendor, attr_manufacturer, product_url, attr_capacity) VALUES ('" + paramsArray.join("','") + "')";
                                    connection.query(SQLquery, function(err, rows, fields) {
                                        if (err) {
                                            error(err);
                                            connection.end();
                    ...
                    Promise.all(promiseArr).then(
                        resolve => {
                            var connection = manage.createConnection(),
                            SQLquery = "INSERT INTO products (name, short_description, description, price, purchase_price, status, meta_title, product_url, img_url, quantity, vendor, category_id, attr_manufacturer, attr_capacity) SELECT name, short_description, description, price, purchase_price, status, meta_title, product_url, img_url, quantity, vendor, category_id, attr_manufacturer, attr_capacity FROM import";
                            connection.connect();
                            connection.query(SQLquery, function(err, rows, fields) {
                                if (err) {
                                    connection.end();
